...
namespace MFPS.Addon.Customizer
{
    public class bl_CustomizerManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private TextMeshProUGUI SeparateText = null;
        [SerializeField]
        private TextMeshProUGUI AutoRotateText = null;
        [SerializeField]
        ...
        public void ShowCustomizerWeapon(bl_Customizer weaponToShow)
        {
            ...
            {
                if (currentCustomizer.Customize)
                {
                    bool open = currentCustomizer.Separate();
                    SeparateText.text = (open) ? "UNITE" : "SEPARATE";
                }           
            }
        ...
        {
            isLoading = false;
            if (bl_CustomizerData.Instance.returnToLobbyAfterSave)
            {
                Exit();
            }
        }
        ...
        private static bl_CustomizerManager _Instance;
        public static bl_CustomizerManager Instance
        {
            get
            {
                if (_Instance == null)
                {
                    _Instance = FindObjectOfType<bl_CustomizerManager>();
                }
                return _Instance;
            }
        }
